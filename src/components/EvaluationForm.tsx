...
export default function EvaluationForm({
  ...
  existingEligibility?: Record<number, Partial<Record<EligibilityKey, boolean>>>;
}) {
  ...
  const [eligibilityByEmployee, setEligibilityByEmployee] = useState<
    Record<number, Partial<Record<EligibilityKey, boolean>>>
  >(existingEligibility);
  ...
  const criteria = useMemo(
    () => criteriaForEmployee(selected),
    [selected, scope, criteriaFixed, criteriaByTeam],
  );
  ...
  async function handleSaveEligibility() {
    ...
    try {
      const current = eligibilityByEmployee[selected.id] ?? {};
      const res = await fetch("/api/eligibility", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          employeeId: selected.id,
          minService: Boolean(current.minService),
          minAttendance: Boolean(current.minAttendance),
          noDisciplinary: Boolean(current.noDisciplinary),
          noPip: Boolean(current.noPip),
          activeEmployee: Boolean(current.activeEmployee),
        }),
      });
      const data = await res.json();
      if (!res.ok) {
        setEligibilityError(data.error ?? "Failed to save eligibility.");
        return;
      }
      setEligibilityStatus("Eligibility saved.");
    } finally {
  ...
  return (
    <div className="grid sm:grid-cols-[16rem_1fr] gap-8">
      <aside>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-6">
              {criteria.map((c) => (
                <div
                  key={c.key}
                  className="grid grid-cols-[1fr_auto] sm:grid-cols-[1fr_11rem] gap-3 items-center py-2 border-b border-black/5 last:border-b-0"
                >
            ...
            {canEditEligibility && (
              <div className="mb-6 pt-3 border-t border-black/10">
                <h3 className="text-sm font-medium mb-1">Eligibility Criteria</h3>
                <p className="text-xs text-neutral-500 mb-3">
                  All criteria must be met for the employee to qualify for the
                  award.
                </p>
                  ...
                  {ELIGIBILITY_ITEMS.map((item) => (
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={Boolean(
                          eligibilityByEmployee[selected.id]?.[item.key],
                        )}
                        onChange={(e) =>
                          setEligibilityByEmployee((prev) => ({
                            ...prev,
                            [selected.id]: {
                              ...prev[selected.id],
                ...
                <button
                  ...
                  className="mt-3 rounded-md border border-black/10 px-3 py-1.5 text-sm font-medium hover:bg-neutral-50 disabled:opacity-50"
                >
                  {savingEligibility ? "Saving…" : "Save eligibility"}
                </button>
                {eligibilityStatus && (
                  <p className="text-xs text-emerald-600 mt-2">{eligibilityStatus}</p>
                )}
                {eligibilityError && (
                  <p className="text-xs text-red-600 mt-2">{eligibilityError}</p>
                )}
              </div>
            )}
